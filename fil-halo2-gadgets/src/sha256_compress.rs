...
use crate::{
    ...
    convert_assigned_f,
    sha256::table16::{ROUNDS as NUM_ROUNDS, ROUND_CONSTANTS},
...
// Maximum number of inputs to `Sha256CompressChip::modular_sum()`.
...
#[derive(Clone)]
pub struct Sha256CompressConfig<F: FieldExt, const BIT_COLS: usize> {
...
pub struct Sha256CompressChip<F: FieldExt, const BIT_COLS: usize> {
    config: Sha256CompressConfig<F, BIT_COLS>,
}

impl<F: FieldExt, const BIT_COLS: usize> ColumnCount for Sha256CompressChip<F, BIT_COLS> {
    fn num_cols() -> NumCols {
        NumCols {
...
impl<F: FieldExt, const BIT_COLS: usize> Sha256CompressChip<F, BIT_COLS> {
    pub fn construct(config: Sha256CompressConfig<F, BIT_COLS>) -> Self {
        Sha256CompressChip { config }
    }

    // # Side Effects
    //
    // - First `Sha256CompressChip::num_cols().advice_eq` `advice` will be equality enabled.
    // - `fixed` will be constant (and equality) enabled.
    pub fn configure(
        meta: &mut ConstraintSystem<F>,
        advice: &[Column<Advice>],
        fixed: &Column<Fixed>,
    ) -> Sha256CompressConfig<F, BIT_COLS> {
    ...
    pub fn hash(
        ...
        layouter.assign_region(
            ...
            |mut region| {
                ...
                let mut state = self.assign_iv(&mut region, &mut offset)?;
...
#[inline]
pub fn sha256_padded_value_blocks(
    blocks: &[[Value<u32>; BLOCK_WORDS]],
) -> [Value<u32>; STATE_WORDS] {
    let padded_preimage_words: Vec<Value<u32>> = blocks.iter().cloned().flatten().collect();
    sha256_padded_value_words(&padded_preimage_words)
}

pub fn sha256_padded_value_words(preimage: &[Value<u32>]) -> [Value<u32>; STATE_WORDS] {
...
mod tests {
    ...
    type Chip = Sha256CompressChip<Fp, BIT_COLS>;
    type Config = Sha256CompressConfig<Fp, BIT_COLS>;

    #[test]
    #[ignore]
    fn test_sha256_compress_chip_assign_word() {
        #[derive(Clone)]
        struct MyCircuit {
            a: Value<u32>,
        }
        ...
        impl Circuit<Fp> for MyCircuit {
            ...
            fn configure(meta: &mut ConstraintSystem<Fp>) -> Self::Config {
                let (advice, _, fixed, _) = ColumnBuilder::new()
                    .with_chip::<Chip>()
                    .create_columns(meta);
                Chip::configure(meta, &advice, &fixed[0])
            }
            ...
            fn synthesize(
                ...
                layouter.assign_region(
                    ...
                    |mut region| {
                        ...
                        assert_eq!(offset - offset_before, Chip::word_rows());
                        assert!(
    ...
    #[test]
    #[ignore]
    fn test_sha256_compress_chip_modular_sum() {
        ...
        impl Circuit<Fp> for MyCircuit {
            ...
            fn synthesize(
                ...
                let sum_rows = Chip::sum_rows();
                let sum_with_decomp_rows = sum_rows + Chip::word_rows();
        ...
        impl CircuitSize<Fp> for MyCircuit {
            fn num_rows(&self) -> usize {
                4 * Chip::sum_rows() + 2 * Chip::word_rows() + 1
            }
        }
    ...
    #[test]
    #[ignore]
    fn test_sha256_compress_chip_sigma() {
        #[derive(Clone)]
        struct MyCircuit {
            word: Value<u32>,
        }
    ...
    #[test]
    #[ignore]
    fn test_sha256_compress_chip_ch_maj() {
        #[derive(Clone)]
        struct MyCircuit {
            word_1: Value<u32>,
            word_2: Value<u32>,
            ...
            fn synthesize(
                ...
                layouter.assign_region(
                    ...
                    |mut region| {
                        ...
                        assert_eq!(offset - offset_before, Chip::ch_maj_rows());
                        self.word_1
                            .zip(self.word_2)
                            .zip(self.word_3)
        ...
        impl CircuitSize<Fp> for MyCircuit {
            fn num_rows(&self) -> usize {
                6 * Chip::word_rows() + 2 * Chip::ch_maj_rows()
            }
        }
    ...
    fn test_sha256_compress_chip() {
        ...
        impl Circuit<Fp> for MyCircuit {
            ...
            fn synthesize(
                ...
                let digest = Chip::construct(config).compress_unassigned_state(
                    layouter,
                    &self.block,
                    &state_in,
                )?;

                let digest_expected = sha256_padded_value_words(&self.block);
    ...
    #[test]
    fn test_sha256_compress_chip_pi_state() {
        #[derive(Clone)]
        struct MyConfig {
            compress: Config,
            pi: Column<Instance>,
        ...
        impl Circuit<Fp> for MyCircuit {
            ...
            fn configure(meta: &mut ConstraintSystem<Fp>) -> Self::Config {
                ...
                let compress = Chip::configure(meta, &advice, &fixed[0]);
            ...
            fn synthesize(
                ...
                let digest = Chip::construct(compress_config).compress_pi_state(
                    layouter,
                    &self.block,
                    pi_col,
                    pi_rows,
    ...
    #[test]
    fn test_sha256_compress_chip_hash() {
        // Configures the number of preimage blocks.
        ...
        impl Circuit<Fp> for MyCircuit {
            ...
            fn synthesize(
                ...
                let digest = Chip::construct(config).hash(layouter, &self.blocks)?;

                let digest_expected = sha256_padded_value_blocks(&self.blocks);
        ...
        impl CircuitSize<Fp> for MyCircuit {
            fn num_rows(&self) -> usize {
                Chip::hash_rows(self.blocks.len())
            }
        }
