...
use fil_halo2_gadgets::{
    ...
    sha256::get_padding,
    sha256_compress::{
        sha256_padded_value_blocks, sha256_padded_value_words, BLOCK_WORDS, PAD_WORDS, STATE_WORDS,
    },
...
// Configures the number of columns used by `Sha256CompressChip`.
const BIT_COLS: usize = 16;

type Sha256CompressChip = fil_halo2_gadgets::sha256_compress::Sha256CompressChip<Fp, BIT_COLS>;
type Sha256CompressConfig = fil_halo2_gadgets::sha256_compress::Sha256CompressConfig<Fp, BIT_COLS>;
...
fn sha256_chip_compute_k() {
    ...
    for num_blocks in PREIMAGE_BLOCK_SIZES {
        ...
        let expected_digest = sha256_padded_value_words(&padded_preimage_words);
...
fn sha256_compress_chip_compute_k() {
    #[derive(Clone)]
    struct MyCircuit {
    ...
    impl Circuit<Fp> for MyCircuit {
        type Config = Sha256CompressConfig;
        type FloorPlanner = SimpleFloorPlanner;
        ...
        fn configure(meta: &mut ConstraintSystem<Fp>) -> Self::Config {
            let (advice, _, fixed, _) = ColumnBuilder::new()
                .with_chip::<Sha256CompressChip>()
                .create_columns(meta);
            Sha256CompressChip::configure(meta, &advice, &fixed[0])
        }
        ...
        fn synthesize(
            ...
            let digest = Sha256CompressChip::construct(config).hash(layouter, &self.blocks)?;
    ...
    impl CircuitSize<Fp> for MyCircuit {
        fn num_rows(&self) -> usize {
            Sha256CompressChip::hash_rows(self.blocks.len())
        }
    }

    println!("Sha256CompressChip (BIT_COLS = {}):", BIT_COLS);
    ...
    for num_blocks in PREIMAGE_BLOCK_SIZES {
        ...
        let expected_digest = sha256_padded_value_blocks(&padded_preimage_blocks);
...
fn main() {
    ...
    sha256_compress_chip_compute_k();
}
